...
function _listOpencodeProjects() {
    if (!DatabaseSync) {
...
function setFileCache(filePath, result) {
...
function safeResolve(file, root) {
    if (!file)
...
function safeNoteResolve(filePath, root) {
    if (!filePath || !root)
        throw new Error("Path required");
...
function getRawDirContents(dir) {
    ...
    if (!entry)
        return null;
    try {
        if (fs.statSync(dir).mtimeMs === entry.mtime)
            return entry;
    }
    catch { }
...
function walkShallow(dir, offset = 0) {
    ...
    if (!contents) {
        ...
        for (const file of entries) {
            ...
            if (stat.isDirectory())
                dirs.push({ type: "dir", name: file, path: full });
            else
                files.push({ type: "file", name: file, path: full });
        }
        dirs.sort((a, b) => a.name.localeCompare(b.name));
        files.sort((a, b) => a.name.localeCompare(b.name));
        setRawDirCache(dir, dirs, files);
        contents = { dirs, files, mtime: 0 };
    }
    const { dirs, files } = contents;
    const slice = files.slice(offset, offset + DIR_FILE_LIMIT);
    const result = [...(offset === 0 ? dirs : []), ...slice];
...
function searchWalk(dir, query, type, results) {
...
app.post("/api/open-in-explorer", (req, res) => {
    const targetPath = req.body?.path || getSessionRoot(req);
    if (!targetPath)
...
app.get("/api/tree", (req, res) => {
    try {
        ...
        const targetDir = req.query.path
            ? safeResolve(req.query.path, sessionRoot)
            : sessionRoot;
...
app.get("/api/file", async (req, res) => {
    try {
        const file = safeResolve(req.query.path, getSessionRoot(req));
        const ext = path.extname(file).toLowerCase();
        ...
        if (ext === ".md") {
            ...
            const rendered = md.render(raw).replace(/<img([^>]*)\ssrc="([^"]+)"/g, (_match, attrs, src) => {
...
function isFalseDot(textBeforeDot) {
    const token = (textBeforeDot.match(/(\S+)\s*$/) || [])[1] || "";
    // Roman numerals: I II III IV V VI VII VIII IX X XI XII ...
    if (/^M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/i.test(token) && token.length <= 6)
        return true;
    ...
    if (ABBREV_PREFIX.test(textBeforeDot))
        return true;
    return false;
}
...
function reconstructSentences(lines) {
    const out = [];
    let cur = "";
    for (const line of lines) {
        if (!line.trim()) {
            if (cur.trim()) {
                out.push(...splitAtSentences(cur));
                cur = "";
            }
            continue;
        }
...
function isMathSegment(text) {
    if (text.length < 3)
        return false;
    if (/[∫∑∏∂∇±×÷≤≥≠≈∞√∈∉⊂⊃∪∩αβγδεζηθλμνξπρστυφχψω]/.test(text))
        return true;
    if (/\\[a-zA-Z]+[\s\{]/.test(text))
        return true;
    if (/\$[^$]{1,100}\$/.test(text))
        return true;
    if (text.length < 20 && /^[\s\d\w\+\-\*\/\=\(\)\[\]\{\}\^\._,:<>|]+$/.test(text))
        return true;
    return false;
}
...
app.post("/api/translate", async (req, res) => {
    const { text, from = "auto", to = "ko" } = req.body;
    ...
    try {
        ...
        const translated = d[0].map((chunk) => chunk[0]).filter(Boolean).join("");
        if (!translated)
...
function mkContainer(type, defaultTitle) {
    return {
        render(tokens, idx) {
...
app.post("/api/notes/project", (req, res) => {
    const sessionRoot = getSessionRoot(req);
    if (!sessionRoot)
        return res.status(400).json({ error: "no project root" });
...
app.get("/api/notes", (req, res) => {
    try {
        const sessionRoot = getSessionRoot(req);
        const notePath = safeNoteResolve(req.query.path, sessionRoot);
        if (!fs.existsSync(notePath))
            return res.json({ content: null });
        res.json({ content: fs.readFileSync(notePath, "utf8") });
    }
