...
export function extractSymbols(code, lang) {
    if (lang === "markdown")
        return extractMarkdown(code);
    if (lang === "python")
        return extractPython(code);
    if (lang === "go")
        return extractGo(code);
    if (lang === "c")
        return extractC(code);
    const cfg = LANG_CONFIGS[lang];
    return cfg ? extractBraced(code, cfg) : [];
}
// ── Markdown ─────────────────────────────────────────────────────────────────
function extractMarkdown(code) {
...
function extractBraced(code, cfg) {
    const symbols = [];
    const lines = code.split("\n");
    let depth = 0;
