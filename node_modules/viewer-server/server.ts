      ...
      rows
        ...
        .map(r => ({
          ...
          iconColor:
            r.icon_color || null,
          timeUpdated:
            Number(r.time_updated) || 0,
          timeCreated:
            Number(r.time_created) || 0,
        }))
...
function shutdownServer(
  ...
  for (const client of wss.clients) {
    try {
      client.send(
        JSON.stringify({ type: "closing" }),
      )
      client.close()
    }
    catch (err) {
      console.error(
        "[viewer:ws client close failed]",
        err,
      )
    }
  }
  ...
  try {
    ...
    console.error(
      "[viewer:parent watch cleanup failed]",
      err,
    )
  }
  ...
  console.error(
  "[viewer:http server close failed]",
  err,
  )
  }
...
async function ensureHighlighter() {
  ...
  try {
    ...
    console.error(
      "[viewer:highlighter init failed]",
      err,
    )
    highlighter = null
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (ext === ".md") {
        ...
        // 3. 심볼 추출 (헤더 기반)
        const tokens = mdWithShiki.parse(raw, {});
        const symbols = [];
...
function encodePlantUml6Bit(value) {
  return PLANTUML_ALPHABET[
    value & 0x3f
  ]
}

function appendPlantUmlEncodedBytes(
  b1,
  b2,
  b3,
) {
...
function startServer(retry = true) {
  ...
  // @ts-ignore
    ...
    wss.clients.forEach((ws: any) => {
      ...
      console.log(
        `[viewer] running at http://0.0.0.0:${PORT} (also http://127.0.0.1:${PORT})`,
      )
